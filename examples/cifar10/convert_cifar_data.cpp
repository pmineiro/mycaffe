...
void convert_dataset(const string& input_folder, const string& output_folder) {
  ...
  for (int fileid = 0, itemid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    // Open files
    ...
    if (! data_file.good ()) {
      std::cerr << "warning: Unable to open train file #" << fileid + 1 << std::endl;
    }
  ...
  std::ifstream data_file((input_folder + "/test_batch.bin").c_str(),
      std::ios::in | std::ios::binary);
  if (! data_file.good ()) {
    std::cerr << "warning: Unable to open test file." << std::endl;
  }
  for (int itemid = 0; data_file.good (); ++itemid) {
    read_image(&data_file, &label, str_buffer);
    if (data_file.good ()) { 
      datum.set_label(label);
      datum.set_data(str_buffer, kCIFARImageNBytes);
